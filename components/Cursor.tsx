...
function getColor(el: HTMLElement): string {
  if (el.closest("[data-cursor-light]")) return "#aaaaaa";
  return "#d4160c";
}
...
const SHAPES: Record<CursorState, { width: number; height: number; borderRadius: number; ring: boolean }> = {
  default: { width: 28, height: 28, borderRadius: 999, ring: false },
  hover:   { width: 40, height: 40, borderRadius: 999, ring: true  },
  text:    { width: 3,  height: 32, borderRadius: 999, ring: false },
  down:    { width: 16, height: 16, borderRadius: 999, ring: false },
};
...
export function Cursor() {
  ...
  const [color, setColor]   = useState("#d4160c");
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      setColor(getColor(target));

      const next = loop.isSelectingText && loop.isDown
        ? "text"
        : getStateFor(target, loop.isDown);
      applyState(next);
    };
      ...
      <div ref={stretchRef} style={{ display: "flex" }}>
        <motion.div
          animate={{
            ...
            backgroundColor: s.ring ? "transparent" : color,
            border:          s.ring ? `2px solid ${color}` : "none",
            opacity:      visible ? 1 : 0,
          }}
          transition={
            isDown
